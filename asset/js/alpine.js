...
e = this, t = function () {
	...
	function n(n) {
		for (var i = 1; i < arguments.length; i++) {
			...
			i % 2 ? t(Object(r), !0)
				.forEach((function (t) {
					e(n, t, r[t])
				})) : Object.getOwnPropertyDescriptors ? Object.defineProperties(n, Object.getOwnPropertyDescriptors(r)) : t(Object(r))
					.forEach((function (e) {
	...
	function k(e, t, n, i, r, s, o) {
		...
		const a = e.style.opacity,
			...
			m = {
				...
				during() {
					f && (e.style.transformOrigin = s.origin), e.style.transitionProperty = [d ? "opacity" : "", f ? "transform" : ""].join(" ")
						.trim(), e.style.transitionDuration = s.duration / 1e3 + "s", e.style.transitionTimingFunction = "cubic-bezier(0.4, 0.0, 0.2, 1)"
				},
				show() {
					n()
				},
	...
	function $(e, t, n, i) {
		...
		e.__x_transition = {
			...
			let n = 1e3 * Number(getComputedStyle(e)
				.transitionDuration.replace(/,.*/, "")
				.replace("s", ""));
			0 === n && (n = 1e3 * Number(getComputedStyle(e)
				.animationDuration.replace("s", ""))), t.show(), e.__x_transition.nextFrame = requestAnimationFrame((() => {
	...
	function j(e, t, i, r, s) {
		...
		let a = D("function" == typeof i ? e.evaluateReturnExpression(t, i) : i),
			l = function (e, t, n, i) {
				...
				return P(s) && s >= 0 && (s = Array.from(Array(s)
					.keys(), (e => e + 1))), s
			}(e, t, a, s),
			c = t;
		l.forEach(((i, o) => {
	...
	function D(e) {
		...
		let r = n[1].trim()
			.replace(/^\(|\)$/g, ""),
			s = r.match(t);
		return s ? (i.item = r.replace(t, "")
			.trim(), i.index = s[1].trim(), s[2] && (i.collection = s[2].trim())) : i.item = r, i
	}
	...
	function T(e, t, n, r, o, l, c) {
		...
		if ("value" === n) {
			...
			if (void 0 === u && String(r)
				.match(/\./) && (u = ""), "radio" === t.type) void 0 === t.attributes.value && "bind" === l ? t.value = u : "bind" !== l && (t.checked = s(t.value, u));
			...
			if (Array.isArray(u)) {
				...
				t.setAttribute("class", i(e.concat(u))
					.join(" "))
			} else if ("object" == typeof u) Object.keys(u)
				.sort(((e, t) => u[e] - u[t]))
				.forEach((e => {
					u[e] ? b(e)
						.forEach((e => t.classList.add(e))) : b(e)
							.forEach((e => t.classList.remove(e)))
				}));
			else {
				const e = t.__x_original_classes || [],
					n = u ? b(u) : [];
				t.setAttribute("class", i(e.concat(n))
					.join(" "))
	...
	function N(e, t, n, i, r, s = {}) {
		// 创建事件监听器的配置对象，根据传入的选项设置是否使用被动监听
		const o = {
		...
		if (i.includes("camel") && (n = a(n)), i.includes("away")) {
			...
			u = document;
			l = a => {
				// 如果点击目标不在指定的元素内，或者元素宽高为 0，则执行事件处理函数
			...
			l = a => {
				...
				u !== window && u !== document || document.body.contains(t) ? function (e) {
					...
					// 过滤出非修饰键
					return n = n.filter((e => !i.includes(e))), !(i.length > 0 && i.filter((t => ("cmd" !== t && "super" !== t || (t = "meta"), e[`${t}Key`])))
						.length === i.length && n[0] === R(e.key))
				}(a, i) || (i.includes("prevent") && a.preventDefault(), i.includes("stop") && a.stopPropagation(), i.includes("self") && a.target !== t) || z(e, r, a)
					.then((e => {
	...
	function R(e) {
		switch (e) {
			...
			case "Spacebar":
				return "space";
			default:
				return e && e.replace(/([a-z])([A-Z])/g, "$1-$2")
					.replace(/[_\s]/, "-")
					.toLowerCase()
		}
	}
	...
	function F(e, t, n) {
		return "radio" === e.type && (e.hasAttribute("name") || e.setAttribute("name", n)), (n, i) => {
			...
			if ("select" === e.tagName.toLowerCase() && e.multiple) return t.includes("number") ? Array.from(n.target.selectedOptions)
				.map((e => I(e.value || e.text))) : Array.from(n.target.selectedOptions)
	...
	// 定义一个函数 I，用于将参数转换为浮点数，如果无法转换，则返回原始参数
	function I(e) {
		// 尝试将参数转换为浮点数，并赋值给变量 t
		const t = e ? parseFloat(e) : null;
		// 如果 t 是一个有效的数字，则返回 t，否则返回原始参数 e
		return P(t) ? t : e;
	}

	// 对象方法解构
	const {
		isArray: M // 判断一个值是否为数组的方法
	} = Array,
		{
			// 对象操作方法
			getPrototypeOf: B,
	...
	class ye {
		...
		getReadOnlyProxy(e) {
			...
			return this.valueIsObservable(t) ? this.getReactiveState(e, t)
				.readOnly : t
		}
		unwrapProxy(e) {
			return re(e)
	...
	class be {
		constructor(e, t = null) {
			...
			let s = {
				...
				o = t ? t.$el : this.$el;
			Object.entries(ge.magicProperties)
				.forEach((([e, t]) => {
		...
		wrapDataInObservable(e) {
			...
			return function (e, t) {
				...
				t.watchers[i] ? t.watchers[i].forEach((t => t(e[i]))) : Array.isArray(e) ? Object.keys(t.watchers)
					.forEach((n => {
						let r = n.split(".");
		...
		initializeElement(e, t, n = !0) {
			e.hasAttribute("class") && h(e, this)
				.length > 0 && (e.__x_original_classes = b(e.getAttribute("class"))), n && this.registerListeners(e, t), this.resolveBoundAttributes(e, !0, t)
		}
		updateElements(e, t = (() => { })) {
		...
		executeAndClearRemainingShowDirectiveStack() {
			this.showDirectiveStack.reverse()
		...
		resolveBoundAttributes(element, isStatic = false, component) {
			// 从元素中获取所有x-data绑定的属性对象
			let bindings = getXDataBindings(element, this);
			...
			bindings.forEach(({
				...
				switch (bindingType) {
					case "model":
						// 处理双向绑定
						handleModelBinding(this, element, "value", bindingExpression, component, bindingType, bindingModifiers);
						break;
					case "bind":
						// 处理其他属性绑定
						if (element.tagName.toLowerCase() === "template" && bindingValue === "key") return; // 忽略key绑定
						handleModelBinding(this, element, bindingValue, bindingExpression, component, bindingType, bindingModifiers);
						break;
					case "text":
						// 处理文本绑定
						var evaluatedText = this.evaluateReturnExpression(element, bindingExpression, component);
						setTextContent(element, evaluatedText, bindingExpression);
						break;
					case "html":
						// 处理HTML绑定
						setInnerHtmlContent(this, element, bindingExpression, component);
						break;
					case "show":
						// 处理显示/隐藏绑定
						evaluatedText = this.evaluateReturnExpression(element, bindingExpression, component);
						handleShowDirective(this, element, evaluatedText, bindingModifiers, isStatic);
						break;
					case "if":
						// 处理条件渲染绑定
						if (bindings.some((b) => b.type === "for")) return; // 如果存在for绑定，则忽略条件渲染绑定
						evaluatedText = this.evaluateReturnExpression(element, bindingExpression, component);
						handleIfDirective(this, element, evaluatedText, isStatic, component);
						break;
					case "for":
						// 处理列表渲染绑定
						handleForDirective(this, element, bindingExpression, isStatic, component);
						break;
					case "cloak":
						// 移除x-cloak属性
